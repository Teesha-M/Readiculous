...
const AddBook = () => {
    ...
    const submit = async (event) => {
        ...
        const bookData = { ...Values, url: formData.imageUrls[0] };

        try {
            await axios.post(`${process.env.REACT_APP_BASE_URL}/addbook`, bookData, { headers });
    ...
    return (
        <div className="bg-green-200 min-h-screen flex flex-col items-center py-8">
            ...
            <form onSubmit={submit} className="bg-white w-[90%] md:w-[50%] p-6 rounded-lg shadow-lg flex flex-col gap-4">
                ...
                <input type="text" name="title" value={Values.title} onChange={change} className="bg-green-100 rounded p-2 border border-green-500 text-black" />

                <label className="text-green-900 font-semibold">Author:</label>
                <input type="text" name="author" value={Values.author} onChange={change} className="bg-green-100 rounded p-2 border border-green-500 text-black" />
                ...
                <input type="text" name="price" value={Values.price} onChange={change} className="bg-green-100 rounded p-2 border border-green-500 text-black" />
                ...
                <textarea name="desc" value={Values.desc} onChange={change} className="bg-green-100 rounded p-2 border border-green-500 text-black" />

                <label className="text-green-900 font-semibold">Upload Image:</label>
                <input type="file" accept="image/*" onChange={(e) => setFile(e.target.files[0])} className="bg-green-100 rounded p-2 border border-green-500 text-black" />
                ...
                {formData.imageUrls.length > 0 && (
                    <div className="flex items-center gap-2">
                        ...
                        <button type="button" onClick={() => setFormData({ imageUrls: [] })} className="text-red-600">Remove</button>
                    </div>
                )}

                <button type="submit" className="mt-4 py-2 bg-green-700 text-white w-full rounded-lg hover:bg-green-800 transition">Add Book</button>
            </form>
        </div>
    );
};
